...
void Camera::yawLookAt(float degree) { yaw = fmod(yaw + degree + 360.0f, 360.0f); }

void Camera::pitchLookAt(float degree) { pitch = std::clamp(pitch + degree, -89.0f, 89.0f); }

void Camera::rollLookAt(float degree) { roll = std::clamp(roll + degree, -89.0f, 89.0f); }
...
math::vec3 &Camera::getPos() { return pos; }
