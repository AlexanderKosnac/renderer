...
Rasterizer::Rasterizer(Framebuffer &fb, Scene &scene) : framebuffer(fb), scene(scene) { updateProjectionMatrix(); }
...
void Rasterizer::render() {
    ...
    for (auto object : scene.getObjects()) {
        ...
        for (auto triangle : object.mesh.getTriangles()) {
            ...
            if (math::dotVec3(normal, cam) > 0.0f)
                continue;
    ...
    for (modelling::Triangle &triangle : depthClippedTriangles) {
        ...
        for (int p = 0; p < 4; p++) {
            ...
            while (nNewTriangles > 0) {
                ...
                switch (p) {
                case 0:
                    nTrisToAdd = modelling::clipTriangleByPlane(candidate, math::vec3(0.0f, 0.0f, 0.0f), math::vec3(0.0f, 1.0f, 0.0f), t[0], t[1]);
                    break; // Right
                case 1:
                    nTrisToAdd = modelling::clipTriangleByPlane(candidate, math::vec3(0.0f, framebuffer.height - 1, 0.0f), math::vec3(0.0f, -1.0f, 0.0f), t[0], t[1]);
                    break; // Top
                case 2:
                    nTrisToAdd = modelling::clipTriangleByPlane(candidate, math::vec3(0.0f, 0.0f, 0.0f), math::vec3(1.0f, 0.0f, 0.0f), t[0], t[1]);
                    break; // Left
                case 3:
                    nTrisToAdd = modelling::clipTriangleByPlane(candidate, math::vec3(framebuffer.width - 1, 0.0f, 0.0f), math::vec3(-1.0f, 0.0f, 0.0f), t[0], t[1]);
                    break; // Bottom
                }
...
void Rasterizer::updateProjectionMatrix() { scene.getCamera().setProjectionMatrix(projectionMatrix); }
...
void fillTriangleBarycentric(Framebuffer &framebuffer, modelling::Triangle &t) {
    ...
    for (int x = minx; x <= maxx; x++) {
        for (int y = miny; y <= maxy; y++) {
            ...
            if (l0 > 0 && l1 > 0 && l2 > 0) {
                math::vec3 color = math::vec3(c0.x * l0 + c1.x * l1 + c2.x * l2, c0.y * l0 + c1.y * l1 + c2.y * l2, c0.z * l0 + c1.z * l1 + c2.z * l2);
